...
# Build Model
## H2O
Since this data set is huge and my system has limited hardware resources, training a classification ML algorithm will crash my system. But no worries we have H2O for our rescue. For those who are not aware of H2O here is the small description of it. `H2O is open-source ML platform for big-data analysis. It is produced by the company H2O.ai. It uses highly scalable in-memory processing for huge data without degrading computational accuracy. It is available as a package in R and provides web UI ‘H2O Flow’ for building models.`

## Selection of predictors
we will use certain variables based on above analysis as predictors for ML algorithm. 
We shall then convert suitable character variables to factors/ categorical variables.

```{r split1}
#colnames(loan1)
model.vars = c("loan_status_binary","grade","int_rate","home_ownership","purpose",
               "dti_range_100","delinq_2yrs","emp_length")
#loan_amnt, term, annual_inc,state, total_acc

loan_final<-loan1[,..model.vars]
loan_final<-loan_final %>% mutate_if(is.character,as.factor)  #convert char to factor type
sapply(loan_final, class)

#train.sub <- loan2[train.ind, c('loan_status_binary', 'state_mean_int', 'home_ownership', 'log_annual_inc', 'dti','term', 'loan_amnt', 'total_acc', 'tot_cur_bal', 'open_acc')]# 'inq_last_6mths')]'purpose', 'total_rev_hi_lim', 'pub_rec', 'acc_now_delinq', 'emp_length', 'grade','sub_grade', 'int_rate', 'delinq_2yrs')], 'funded_amnt', 'funded_amnt_inv','installment', 'verification_status','pymnt_plan', 'revol_bal', 'initial_list_status')]

```

## Build/train/validate classification models
...
train.ind <-createDataPartition(loan_final$loan_status_binary,p = .7,list = F)
loan_train<-as.data.table(loan_final[train.ind,])
loan_validate<-as.data.table(loan_final[-train.ind,])
dim(loan_train); dim(loan_validate)
```
```{r split1}
#set.seed(1)
...
## Initialize H2O in R
We shall first initialize H2O on our machine. Since my laptop has RAM of 4GB, I will use max_mem_size = “4g” setting while initialization. Since, Kaggle provides 17GB of RAM, I will use max_mem_size = “15g” setting here. We shall also convert training and validation sets to equivalent H2O type. Thereafter it can be used with H2O to build and validate models.
```{r h2o}
...
colnames(train)
```
Column#1 is target variable and Column#2 to Column#8 are predictors.

## Logistic Regression using H2o
We will use Logistic Regression model to predict our target variable defaulted as this is the most commonly used classification model.
```{r log reg}
model.glm <- h2o.glm(x=c(2:8),y=1,training_frame = train,validation_frame = validation,nfolds = 5,family = "binomial",seed = 786,link = "logit",compute_p_values = T,lambda = 0,standardize = F,remove_collinear_columns = T)
```
### Interpretation of Coefficients
```{r log reg1}
knitr::kable(as.data.frame(model.glm@model$coefficients_table)) %>% kable_styling() %>% scroll_box(width = "500px", height = "200px")
```
### Performance
```{r log reg2}
model.glm@model$validation_metrics
plot(h2o.performance(model.glm,validation),type='roc')
```
### Observation
...
## Random Forest
```{r RF}
model.rf<-h2o.randomForest(x=c(2:8),y=1,training_frame = train,validation_frame = validation, ntrees = 300,nbins_cats = 50,seed = 786,categorical_encoding = "Enum")
```
### Variable Importance
```{r rf1}
...
### Performance
```{r rf2}
model.rf@model$validation_metrics
#plot(h2o.performance(model.rf,validation),type='roc')
h2o.confusionMatrix(model.rf,thresholds=.1,newdata=validation)
```

## Gradient Boosting Machine
```{r gbt}
model.gbm<-h2o.gbm(x=c(2:8),y=1,training_frame = train,validation_frame = validation,ntrees = 300,nbins_cats = 50,seed = 786,categorical_encoding = "Enum")
```
### Variable Importance
```{r gbt1}
...
### Performance
```{r gbt3}
model.gbm@model$validation_metrics
plot(h2o.performance(model.gbm,validation),type='roc')
h2o.confusionMatrix(model.gbm,thresholds=.1,newdata=validation)
```
### Conclusion
...
# Prediction on Test data set of 2016-2017
## Feature Engineering
## Predicing Defaulted Loans
